...
const _ = require('lodash'),
  ...
  encodeDatabaseRange = command => {
    console.log('address', command.address);
    console.log('a1', (command.address ? command.address.substr(0, 2) : '00'));
    console.log('a2', (command.address ? command.address.substr(2, 2) : '00'));
    return '00' +
    '00' +
  ...
  sd = (name, command1, command2 = '00') => {
    const encoder = command =>
      encoders['Send INSTEON Standard-length Message']({
        ...
        command1,
        command2: _.isFunction(command2) ? command2(command) : command2 === '00' && command.command2 ? command.command2 : command2,
  ...
  ed = (name, command1, command2 = '00', userData = '', matcher) => {
    const encoder = command => {
      ...
      console.log('userData', _.isFunction(userData), userData(command));
...
im('Beep IM', '77', () => '00'); // Note: the documentation does not mention
                              //       the 00 byte
