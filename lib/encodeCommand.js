...
const toHexSigned = (value = 0, length = 2) => (value < 0) ?
        ...
        toHex(value, length);
const encodeHex = property => command => toHex(command[property] || 0);
...
const ed = (name, command1, command2 = '00', userData = '') => {
...
im('Beep', '77', () => '00');
im('Set Status', '7F', command => (command.status || '00'));
...
// 26 Reserved
sd('Light ON at Ramp Rate', '27', encodeHex('onLevel'));
// 28-2D Deprecated
