...
const _ = require('lodash'),
  ...
  encodeX10Address = command =>
    encodeHex(command.groupNumber) +
    '04' +
    encodeHex(command.houseCode) +
    encodeHex(command.unitCode),

  encodeLEDBrightness = command =>
    '00' +
    '07' +
    encodeHex(command.onLevel),

  encodeDatabaseRange = command =>
    '00' +
    '00' +
    (command.address ? command.address.slice(0, 2) : '00') +
    (command.address ? command.address.slice(2, 2) : '00') +
    (command.numberRecords ? encodeHex(command.numberRecords) : '00'),
  ...
  im = (name, code, data, matcher) => {
    ...
    commandResponseMatchers[name] = matcher ||
      (response =>
       response.command === name && response.ack
      );
  },
...
im('Set Host Device Category', '66', command =>
  ...
  (command.firmware || '00'));
im('Reset the IM', '67'); // TODO: adjust timeout since this can take 20 secs to return
...
im('Beep', '77', () => '00'); // Note: the documentation does not mention
                              //       the 00 byte
...
ed('RF Off', '20', '0C');
ed('RF On', '20', '0D');
ed('Powerline On', '20', '0E');
ed('Powerline Off', '20', '0F');
ed('X10 Off', '20', '12');
ed('X10 On', '20', '13');
...
ed('Set LED Brightness', '2E', '00', encodeLEDBrightness);
...
encodeCommand.encoders = encoders; // TODO: remove after moving to plm2
