...
const { OUTLET_CODES, OUTLET_NAMES, X10_HOUSE_CODES, X10_UNIT_CODES, NAK_ERRORS } =
  require('./constants')
...
const direct = (command1And2, commandName, command2PropertyOrParser) => {
  const command1 = command1And2.substr(0, 2)
...
extendedData('0300', 'Product Data Response', ({ command, response }) => {
  ...
  const deviceCategory = data.substr(8, 2)
  const deviceSubcategory = data.substr(10, 2)
...
extendedData('2E', 'Extended Set/Get', ({ command, response }) => {
  const data = response.data
  const commandTypeByte = data.substr(2, 2)
  ...
  switch (commandTypeByte) {
    ...
    case '01':
      ...
      command.ledBrightness = parseInt(data.substr(16, 2), 16)
      break
    case '04':
...
extendedData('2F', 'Read/Write ALL-Link Database', ({ command, response }) => {
  ...
  switch (commandTypeByte) {
    case '00':
      ...
      command.numberOfRecords = parseInt(data.substr(8, 2), 16)
      break
    case '01':
      command.type = 'Record Response'
      Object.assign(command, createAllLinkRecord(data.substr(10, 16)))
      break
    case '02':
