...
const parseInsteonCommand = (response, previousCommand) => {
  ...
  if (response.allLinkCommand) {
  ...
  } else if (response.messageType === 'allLinkBroadcast') {
    parser = allLinkParsers[response.command1];
  } else if (response.messageType === 'allLinkCleanup') {
    parser = allLinkParsers[response.command1];
    ...
    // TODO: add directNak, allLinkCleanupAck, allLinkCleanupNak
...
addImParser('5C', 'INSTEON 0x5C Message', 18, (response, buffer) => {
  response.address = buffer.substr(0, 4);
  Object.assign(response, parseAllLinkRecord(buffer.substr(4, 16)));
  response.fromAddress = buffer.substr(0, 6);
  response.toAddress = buffer.substr(6, 6);
...
addImParser('75', 'Read 8 bytes from Database', 22, (response, buffer) => {
  response.address = buffer.substr(0, 4);
  response.ack = buffer.substr(4, 2) === '06';
...
addImParser('7F', '7F Command', 4, (response, buffer) => {
  response.data = buffer.substr(0, 2);
  response.ack = buffer.substr(2, 2) === '06';
  return response;
});
