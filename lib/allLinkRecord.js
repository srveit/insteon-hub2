...
const parseAllLinkRecord = buffer => {
  ...
  if (response.inUse) {
    if (response.isController) {
      ...
      response.numberRetries = parseInt(buffer.substr(10, 2));
      response.controllerGroupNumber = parseInt(buffer.substr(14, 2));
    } else {
      response.onLevel = parseInt(buffer.substr(10, 2), 16);
      response.rampRate = parseInt(buffer.substr(12, 2), 16);
      response.responderGroupNumber = parseInt(buffer.substr(14, 2));
    }
